...
export default function ResetPasswordPage({
  ...
  searchParams: { token: string; email_token: string };
}) {
  let tokenHasExpired = false;
  let token;
  let emailToken;
  try {
    emailToken = jwt.verify(
      searchParams["email_token"],
      process.env.JWT_SECRET!
    );
    token = jwt.verify(searchParams.token, process.env.JWT_SECRET!);
  } catch (error) {
    tokenHasExpired = true;
  }

  if (!emailToken) notFound();

  if (tokenHasExpired || !token || typeof token === "string")
    return (
      <div className={styles["confirm-email-page"]}>
        <div className={styles["container"]}>
          ...
          <p>
            ...
            <strong>{(emailToken as JwtPayload).email}</strong> has expired
          </p>
          <p>
            Click on the link below to re-send the verification email to
            complete your signup.
          </p>
          <PasswordResetForm
            email={(emailToken as JwtPayload).email}
            firstname={(emailToken as JwtPayload).firstname}
          />
        </div>
      </div>
    );
  return (
    <div className={styles["reset-password-page"]}>
      <div className={styles["form-container"]}>
        ...
        <ResetPasswordForm email={token.email} />
      </div>
    </div>
  );
}
