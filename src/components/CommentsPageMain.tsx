...
export default function CommentsPageMain({
  ...
  function closeModal() {
    setCommentToView((curr) => ({ ...curr, open: false }));
  }
  ...
  return (
    <>
          ...
          <tbody>
            {paginatedComments.map((comment) => (
              <tr key={comment.id}>
                ...
                <td>
                  <button
                    onClick={() => setCommentToView({ open: true, comment })}
                  >
                    <EyeIcon width={18} height={18} color="#333" />
                  </button>
                </td>
              </tr>
            ))}
      ...
      <ViewCommentModal
        isOpen={commentToView.open}
        comment={commentToView.comment}
